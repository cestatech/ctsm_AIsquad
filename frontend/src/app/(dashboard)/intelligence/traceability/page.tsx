...
const CHAIN_STAGES = [
  { key: "OBJECTIVE", label: "Objective", color: "bg-violet-500", text: "text-violet-900", bg: "bg-violet-50", border: "border-violet-200" },
  { key: "ENDPOINT", label: "Endpoint", color: "bg-blue-500", text: "text-blue-900", bg: "bg-blue-50", border: "border-blue-200" },
  ...
  { key: "TLF", label: "TLF", color: "bg-amber-500", text: "text-amber-900", bg: "bg-amber-50", border: "border-amber-200" },
...
export default function TraceabilityMatrixPage() {
  ...
  const { data: nodesData, isLoading: nodesLoading } = useQuery({
    queryKey: ["graph-nodes-trace", studyId, token],
    queryFn: () =>
  ...
  const { data: edgesData, isLoading: edgesLoading } = useQuery({
    queryKey: ["graph-edges-trace", studyId, token],
    queryFn: () =>
      graphApi.listEdges({ study_id: studyId!, page_size: 500 }, token!),
    enabled: !!token && !!studyId,
    staleTime: 60_000,
  });
        ...
        {!studyId ? (
          <div className="bg-white border border-dashed border-slate-300 px-8 py-16 text-center">
            ...
            <p className="text-slate-400 text-sm">
              Choose a study above to view its traceability matrix.
            </p>
          </div>
        ) : isLoading ? (
          ...
          <>
            ...
            <div className="space-y-4">
              ...
              {CHAIN_STAGES.every((s) => (nodesByType[s.key] ?? []).length === 0) && (
                <div className="bg-white border border-dashed border-slate-300 px-8 py-12 text-center">
                  <p className="font-display font-semibold text-slate-700 mb-1">No nodes registered</p>
                  <p className="text-slate-400 text-sm">
                    No graph nodes have been created for this study yet. They appear as AI agents
                    generate and map clinical data entities.
                  </p>
                </div>
              )}
            </div>
