...
def _check_sdtm(content: dict) -> list[Finding]:
    ...
    if not domains:
        findings.append(_f(
            "SDTM-001", "At least one SDTM domain must be present",
            FAIL, ERROR, "No domains found in SDTM specification"
        ))
        return findings

    for domain_spec in domains:
        domain_name: str = domain_spec.get("dataset", domain_spec.get("domain", "UNKNOWN"))
        ...
        if "STUDYID" not in var_names:
            findings.append(_f(
                "SDTM-002", "STUDYID required in all domains",
                FAIL, ERROR,
                f"STUDYID missing from domain {domain_name}",
                domain=domain_name, variable="STUDYID",
            ))
        else:
            findings.append(_f("SDTM-002", "STUDYID required in all domains",
                               PASS, ERROR, domain=domain_name, variable="STUDYID"))

        # SDTM-003: DOMAIN value must be 2-character uppercase
        if domain_name != "UNKNOWN":
            if not re.fullmatch(r"[A-Z]{2}", domain_name[:2]):
                findings.append(_f(
                    "SDTM-003", "DOMAIN must be 2-character uppercase abbreviation",
                    FAIL, ERROR,
                    f"Domain name '{domain_name}' does not conform to 2-char uppercase rule",
                    domain=domain_name,
                ))
            else:
                findings.append(_f("SDTM-003", "DOMAIN must be 2-character uppercase abbreviation",
                                   PASS, ERROR, domain=domain_name))
        ...
        if domain_name not in trial_design_domains:
            if "USUBJID" not in var_names:
                findings.append(_f(
                    "SDTM-004", "USUBJID required in subject-level domains",
                    FAIL, ERROR,
                    f"USUBJID missing from domain {domain_name}",
                    domain=domain_name, variable="USUBJID",
                ))
            else:
                findings.append(_f("SDTM-004", "USUBJID required in subject-level domains",
                                   PASS, ERROR, domain=domain_name, variable="USUBJID"))

        # SDTM-005: DM domain required variables
        if domain_name == "DM":
            dm_required = {
                "RFSTDTC", "RFENDTC", "SEX", "RACE", "ETHNIC",
                "ARMCD", "ARM", "COUNTRY", "AGE", "AGEU"
            }
            ...
            if missing:
                ...
                findings.append(_f("SDTM-005", "DM domain required variables",
                                   PASS, ERROR, domain="DM"))
        ...
        if domain_name == "AE":
            ...
            if missing:
                ...
                findings.append(_f("SDTM-006", "AE domain required variables",
                                   PASS, ERROR, domain="AE"))
        ...
        for var in variables:
            ...
            if vname.endswith("DTC"):
                ...
                if derivation and "ISO 8601" not in derivation and "YYYY-MM-DD" not in derivation:
                    findings.append(_f(
                        "SDTM-007", "DTC variables must follow ISO 8601 format",
                        FAIL, WARNING,
                        f"{vname} derivation does not reference ISO 8601 format",
                        domain=domain_name, variable=vname,
                    ))
                else:
                    findings.append(_f("SDTM-007", "DTC variables must follow ISO 8601 format",
                                       PASS, WARNING, domain=domain_name, variable=vname))
    ...
    if not any("SDTM" in r for r in reg_refs):
        findings.append(_f(
            "SDTM-010", "CDISC SDTM IG reference required",
            FAIL, ERROR,
...
def _check_adam(content: dict) -> list[Finding]:
    ...
    if not datasets:
        findings.append(_f(
            "ADAM-001", "At least one ADaM dataset must be present",
            FAIL, ERROR, "No datasets found in ADaM specification"
        ))
        return findings
    ...
    if not has_adsl:
        findings.append(_f(
            "ADAM-002", "ADSL dataset required in ADaM submission",
            FAIL, ERROR, "ADSL (Subject Level Analysis Dataset) not found"
        ))
    else:
        findings.append(_f("ADAM-002", "ADSL dataset required in ADaM submission", PASS, ERROR))
    ...
    for ds in datasets:
        ...
        if ds_name == "ADSL":
            ...
            if missing:
                ...
                findings.append(_f("ADAM-003", "ADSL required variables",
                                   PASS, ERROR, domain=ds_name))
        ...
        if ds_name == "ADSL":
            ...
            if not pop_flags:
                findings.append(_f(
                    "ADAM-004", "Population flags required in ADSL",
                    FAIL, ERROR,
                ...
                findings.append(_f("ADAM-004", "Population flags required in ADSL",
                                   PASS, ERROR, domain="ADSL"))

                # ADAM-005: ITTFL must be present
                if "ITTFL" not in flag_vars:
                    findings.append(_f(
                        "ADAM-005", "ITTFL (Intent-to-Treat flag) required in ADSL",
                        FAIL, ERROR,
                        "ITTFL not found in ADSL population_flags",
                        domain="ADSL", variable="ITTFL",
                    ))
                else:
                    findings.append(_f("ADAM-005", "ITTFL required in ADSL",
                                       PASS, ERROR, domain="ADSL", variable="ITTFL"))
        ...
        for var in variables:
            ...
            if not derivation or derivation.strip() == "":
                findings.append(_f(
                    "ADAM-006", "All ADaM variables must have derivation documented",
                    FAIL, ERROR,
                    f"{vname} in {ds_name} has no derivation algorithm",
                    domain=ds_name, variable=vname,
                ))

        # ADAM-007: BDS datasets require PARAM, PARAMCD, AVAL
        bds_indicators = {"ADLB", "ADVS", "ADEFF", "ADRS", "ADPC"}
        is_bds = ds_name in bds_indicators or ds_name.startswith("AD") and ds_name != "ADSL"
        if is_bds and ds_name not in ("ADSL", "ADAE"):
            ...
            if missing:
                ...
                findings.append(_f("ADAM-007", "BDS datasets require PARAM, PARAMCD, AVAL",
                                   PASS, ERROR, domain=ds_name))
        ...
        if ds_name == "ADAE":
            ...
            if missing:
                ...
                findings.append(_f("ADAM-008", "ADAE required variables",
                                   PASS, ERROR, domain="ADAE"))

    # ADAM-009: ADaM IG version documented
    ig_version = content.get("adam_ig_version")
    if not ig_version:
        findings.append(_f(
            "ADAM-009", "ADaM IG version must be documented",
            FAIL, WARNING,
            "adam_ig_version not specified",
        ))
    else:
        findings.append(_f("ADAM-009", "ADaM IG version must be documented", PASS, WARNING))

    # ADAM-010: Traceability notes present
    trace_notes = content.get("traceability_notes", [])
    if not trace_notes:
        findings.append(_f(
            "ADAM-010", "Traceability notes required in ADaM specification",
            FAIL, WARNING,
...
def _check_protocol(content: dict) -> list[Finding]:
    ...
    if not synopsis:
        findings.append(_f("PROT-001", "Protocol synopsis required (ICH E6 §6.1)",
                           FAIL, ERROR, "No synopsis section found"))
    else:
        findings.append(_f("PROT-001", "Protocol synopsis required", PASS, ERROR))

    # PROT-002: Objectives section present
    objectives = content.get("objectives", {})
    if not objectives or (not objectives.get("primary") and not objectives.get("primary_objective")):
        findings.append(_f("PROT-002", "Primary objective required (ICH E6 §6.2)",
                           FAIL, ERROR, "No primary objective documented"))
    else:
    ...
    if not design:
        findings.append(_f("PROT-003", "Study design section required (ICH E6 §6.4)",
                           FAIL, ERROR, "No study_design section found"))
    else:
    ...
    has_inclusion = bool(eligibility.get("inclusion_criteria") or eligibility.get("inclusion"))
    has_exclusion = bool(eligibility.get("exclusion_criteria") or eligibility.get("exclusion"))
    if not has_inclusion or not has_exclusion:
        findings.append(_f("PROT-004", "Inclusion and exclusion criteria required (ICH E6 §6.5)",
                           FAIL, ERROR,
    ...
    primary_endpoint = (
        endpoints.get("primary") or
        endpoints.get("primary_endpoint") or
        content.get("primary_endpoint")
    )
    if not primary_endpoint:
        findings.append(_f("PROT-005", "Primary endpoint required (ICH E6 §6.6)",
                           FAIL, ERROR, "No primary endpoint found"))
    else:
        findings.append(_f("PROT-005", "Primary endpoint documented", PASS, ERROR))

    # PROT-006: Statistical considerations
    stats = (
        content.get("statistical_considerations") or
        content.get("statistics") or
        content.get("statistical_analysis")
    )
    if not stats:
        findings.append(_f("PROT-006", "Statistical considerations required (ICH E6 §6.9)",
                           FAIL, ERROR, "No statistical_considerations section found"))
    else:
        findings.append(_f("PROT-006", "Statistical considerations present", PASS, ERROR))

    # PROT-007: Safety monitoring plan
    safety = content.get("safety_monitoring", {})
    if not safety:
        findings.append(_f("PROT-007", "Safety monitoring section required (ICH E6 §5.18)",
                           FAIL, WARNING, "No safety_monitoring section found"))
    else:
...
def _check_icf(content: dict) -> list[Finding]:
    findings: list[Finding] = []
    sections = {s.get("section", s.get("title", "")): s
    ...
    for element, aliases in required_elements.items():
        ...
        if found:
            ...
            findings.append(_f(rule_id, rule_name, FAIL, ERROR,
                               f"ICF missing required element: {element}"))

    # ICF-009: Reading level
    reading_level = content.get("reading_level") or content.get("target_reading_level", "")
    if reading_level and "8th" not in str(reading_level) and "6th" not in str(reading_level):
        findings.append(_f("ICF-009", "ICF reading level should be 8th grade or lower",
                           FAIL, WARNING,
...
def _check_sap(content: dict) -> list[Finding]:
    ...
    if not populations:
        findings.append(_f("SAP-001", "Analysis populations required (ICH E9 §5.2.2)",
                           FAIL, ERROR, "No analysis populations defined"))
    else:
        findings.append(_f("SAP-001", "Analysis populations defined", PASS, ERROR))

    # SAP-002: Primary endpoint analysis method
    primary = content.get("primary_endpoint_analysis") or content.get("primary_analysis")
    if not primary:
        findings.append(_f("SAP-002", "Primary endpoint analysis method required",
                           FAIL, ERROR, "No primary_endpoint_analysis section found"))
    else:
        findings.append(_f("SAP-002", "Primary endpoint analysis documented", PASS, ERROR))

    # SAP-003: Estimands framework (ICH E9(R1))
    estimands = content.get("estimands", [])
    if not estimands:
        findings.append(_f("SAP-003", "Estimands framework required (ICH E9(R1))",
                           FAIL, ERROR,
        ...
        for i, est in enumerate(estimands):
            for field in ("treatment_condition", "population", "variable", "handling_of_intercurrent_events"):
                if not est.get(field):
                    findings.append(_f(
                        "SAP-004", "Estimand completeness (ICH E9(R1) §A.4)",
                        FAIL, WARNING,
    ...
    if not missing_data:
        findings.append(_f("SAP-005", "Missing data strategy required (ICH E9 §5.3.3)",
                           FAIL, ERROR, "No missing data handling strategy documented"))
    else:
    ...
    if not sensitivity:
        findings.append(_f("SAP-006", "Sensitivity analyses required (ICH E9(R1))",
                           FAIL, WARNING, "No sensitivity_analyses documented"))
    else:
    ...
    if not secondary:
        findings.append(_f("SAP-007", "Secondary endpoints analysis required",
                           FAIL, WARNING, "No secondary endpoint analyses documented"))
    else:
        findings.append(_f("SAP-007", "Secondary endpoints analysis documented", PASS, WARNING))
...
def _check_csr(content: dict) -> list[Finding]:
    ...
    if synopsis:
        for field in ("objectives", "design", "population"):
            if not synopsis.get(field):
                findings.append(_f("CSR-010", "Synopsis completeness (ICH E3 §2)",
                                   FAIL, WARNING,
    ...
    if not appendices:
        findings.append(_f("CSR-011", "Appendices must be listed (ICH E3 §16)",
                           FAIL, WARNING, "No appendices defined"))
    else:
    ...
    if not content.get("ich_e3_compliant"):
        findings.append(_f("CSR-012", "ich_e3_compliant must be true",
                           FAIL, ERROR,
...
def _check_generic(content: dict, artifact_type: str) -> list[Finding]:
    ...
    if not content:
        findings.append(_f("GEN-001", "Artifact content must not be empty",
                           FAIL, ERROR, "Content is empty"))
    else:
    ...
    if doc_type and expected and doc_type != expected:
        findings.append(_f("GEN-002", "document_type must match artifact type",
                           FAIL, WARNING,
                           f"Expected document_type '{expected}', found '{doc_type}'"))
    else:
        findings.append(_f("GEN-002", "document_type consistent with artifact type", PASS, WARNING))

    # GEN-003: Version field present
    if not content.get("version"):
        findings.append(_f("GEN-003", "Version field required",
                           FAIL, WARNING, "No 'version' field in artifact content"))
    else:
    ...
    reg_refs = content.get("regulatory_references", [])
    if not reg_refs:
        findings.append(_f("GEN-004", "Regulatory references required",
                           FAIL, WARNING, "No regulatory_references array found"))
    else:
        findings.append(_f("GEN-004", "Regulatory references documented", PASS, WARNING))
...
def run_cdisc_validation(
    content: dict, artifact_type: str
) -> dict:
    """
    ...
    if checker:
        ...
        findings.append(_f(
            "GEN-005", "Artifact type has no specific CDISC rule set",
            "PASS", INFO,
